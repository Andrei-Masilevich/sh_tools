...
__show_help()
{
   ...
   echo "Decrypt data file with key file or passphrase.                         "
...
main()
{
    ...
    if (( $OPT_MASTER_KEY )); then
        ...
        if [ -n "${__MASTER_PASSPHRASE}" ]; then
            decrypt_file ${DATA_FILE} ${__MASTER_PASSPHRASE}
            __MASTER_PASSPHRASE=
        else
        ...
        if [ -n "${__PASSPHRASE}" ]; then
            decrypt_file ${DATA_FILE} "${__PASSPHRASE}"
            __PASSPHRASE=
        else
        ...
        decrypt_file_with_key_file ${DATA_FILE} ${KEY_FILE}
    fi
}
