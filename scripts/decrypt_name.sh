...
__show_help()
{
   ...
   echo "Decrypt filename with key file or passphrase.                          "
   print_passphrase_requirements
   echo "_______________________________________________________________________"
   echo " -d       Decrypt directory name                                       "
...
main()
{
    ...
    if (( $OPT_MASTER_KEY )); then
        ...
        if [ -n "${__MASTER_PASSPHRASE}" ]; then
            decrypt_name ${DATA_FILE} ${__MASTER_PASSPHRASE} $OPT_IS_DIR
            __MASTER_PASSPHRASE=
        else
        ...
        if [ -n "${__PASSPHRASE}" ]; then
            decrypt_name ${DATA_FILE} "${__PASSPHRASE}" $OPT_IS_DIR
            __PASSPHRASE=
        else
        ...
        decrypt_name_with_key_file ${DATA_FILE} ${KEY_FILE} $OPT_IS_DIR
    fi
