...
main()
{
    ...
    if (( $OPT_RECURSIVELY )); then
        encrypt_folder_recursively ${ARGS} "${PASSPHRASE}"
    else
        encrypt_folder ${ARGS} "${PASSPHRASE}"
    fi
}
